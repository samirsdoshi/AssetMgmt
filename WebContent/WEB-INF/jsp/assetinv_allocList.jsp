...
<head>
    ...
    <title><s:text name="label.assetinv_allocs"/></title>
</head>
<body>
...
<div class="${pageClass}">
<br/>
<h1><s:text name="label.assetinv_allocs"/></h1>
<s:url id="url" action="crudassetinv_alloc_input" >
	<s:param name="alloctype.id" value="%{alloctype.id}"/>
	<s:param name="assetinv.id" value="%{assetinv.id}"/>
</s:url>
<a href="<s:property value="#url"/>">Add New assetinv_alloc</a>
<br/><br/>
<table class="borderAll">
    <tr>
        <th><s:text name="label.assetinv_alloc.assetinv_id"/></th>
        <th><s:text name="label.assetinv_alloc.alloctype_id"/></th>
        <th><s:text name="label.assetinv_alloc.amount"/></th>
        <th>&nbsp;</th>
    </tr>
    <s:iterator value="assetinv_allocs" status="status">
        <tr class="<s:if test="#status.even">even</s:if><s:else>odd</s:else>">
            ...
            <td onclick="javascript:stopPropagation(event);"  class="nowrap">
                <s:url action="crudassetinv_alloc_input" id="url">
                    <s:param name="assetinv_alloc.id" value="id"/>
                </s:url>
                <a href="<s:property value="#url"/>">Edit</a>
                &nbsp;&nbsp;&nbsp;
                <s:url action="crudassetinv_alloc_delete" id="url">
                    <s:param name="assetinv_alloc.id" value="id"/>
                </s:url>
